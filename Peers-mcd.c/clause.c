...
void print_clause(FILE *fp, Clause_ptr c)
{
    ...
    fprintf(fp,"<%d,%d,%d> (wt=%d) [", owner(c->id), birth_place(c->id),
                num(c->id), c->weight);
...
void get_ancestors(Clause_ptr c, Gen_ptr_ptr *id_table, Gen_ptr_ptr *pp)
{
    ...
    if (!c) {
    /* It can be called with c == NULL if find_clause_by_id returned NULL, */
    /* which means a clause was not found. Then we put a NULL in the list  */
    /* and when we reach it in print_proof a message will be printed.      */
	p2 = get_gen_ptr();
	p2->u.c = NULL;
	p2->next = *pp;
	*pp = p2;
	...
	if (!p1 || p1->u.c->id > c->id) {
		...
		*pp = p2;
		...
		if (p1->u.i > 0)	/* it's clause id, not rule code */
