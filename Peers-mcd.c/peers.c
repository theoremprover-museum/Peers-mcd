...
/*************
 *
 *    flag_dep_init() -- initialize global variables, which are
...
void broadcast_clause(Clause_ptr c)
{
    ...
    CLOCK_START(INF_MSG_TIME);
    if (Number_of_peers > 1 && (c->literals->sign || !Flags[OWN_GOALS].val) &&
        (Parms[DECIDE_OWNER_STRAT].val != NO_SUBDIVIDE || c->weight <= Parms[MAX_WEIGHT_SEND].val)) {
