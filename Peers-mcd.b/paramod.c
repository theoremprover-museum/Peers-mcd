...
void paramod(Clause_ptr from, Term_ptr from_atom, Clause_ptr into,
	     Term_ptr into_atom, Term_ptr into_term, int where)
{
    int i;
	...
	for (i = 0; i < into_term->arity; i++)
	    paramod(from, from_atom, into, into_atom, into_term->args[i], ALL);
	}
...
void para_from_into(Clause_ptr c, Clause_ptr d, int where)
{
    ...
    if (from_c)
	paramod(c, cl->atom, d, dl->atom, dl->atom->args[0], where);
	...
	if (c_ext_atom)
	    paramod(c, c_ext_atom, d, dl->atom, dl->atom->args[0], where);
	
	/* Note that when going into extensions, go into top level only. */
	
	if (c_ext_atom && d_ext_atom && where == ALL)
	    paramod(c, c_ext_atom, d, d_ext_atom, d_ext_atom->args[0], TOP_ONLY);
			...
			Fpa_index_ptr idx_into)
{
	...
	if (pos_eq_lit(l)) {
		...
		if (mine(into_clause))	/* Peers-mcd, October 1996 */
