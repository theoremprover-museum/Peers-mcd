	     ...
	     Term_ptr t2, Context_ptr c2, Bt_node_ptr bt)
{
	...
	ac->superset_limit = Parms[AC_SUPERSET_LIMIT].val;
        ...
        printf("    "); print_term(stdout, t1); printf(" [0x%x]\n", (unsigned) c1);
        printf("    "); print_term(stdout, t2); printf(" [0x%x]\n", (unsigned) c2);
	...
	if (continuation) {
		...
		if (ci && VARIABLE(ti)) {
		    ...
		    ci->contexts[vn] = NULL;
#ifdef DEBUG
		    printf("<-<-<- clearing (ci) "); print_term(stdout, ti);
		    printf(" [0x%x]\n", (unsigned) ci);
#endif
		    }
...
#ifdef DEBUG
		    printf("->->->-> binding (c3)"); print_term(stdout, ac->new_terms[i]);
		    printf(" [0x%x]\n", (unsigned) ac->c3);
#endif
		    }
		}
...
#ifdef DEBUG
		    printf("->->->-> binding (ci) "); print_term(stdout, ti);
		    printf(" [0x%x] -> ", (unsigned) ci);
...
#ifdef DEBUG
		    printf("->->->-> binding (c3)"); print_term(stdout, t4);
		    printf(" [0x%x] -> ", (unsigned) ac->c3);
