...
void p_ac_basis(basis, num_basis, m, n)
int basis[][MAX_COEF], num_basis, m, n;
{
    int i, j;
...
int var_check_1(constraints, xy, start, stop)
int constraints[], xy[], start, stop;
{
    int i;
...
int var_check_2(constraints, xy, start, stop)
int constraints[], xy[], start, stop;
{
    int i, j;
...
int add_solution(xy, length, num_basis, basis)
int xy[], length, *num_basis, basis[][MAX_COEF];
{
    int i;
    for (i = 0; i < *num_basis; i++)
...
int a_in_bounds(ab,xy,max_y,d,e,m,n,xypos,max_a,max_b,suma,constraints)
int ab[], xy[], max_y[], d[][MAX_COEF], e[][MAX_COEF], constraints[];
int m, n, xypos, max_a, max_b, suma;
{
    int i, j, f, bsum;
...
int b_in_bounds(xy, max_y, constraints, xypos, suma, sumb, m, n)
int xy[], max_y[], constraints[], xypos, suma, sumb, m, n;
{
...
int dio(ab,m,n,constraints,basis,num_basis)
int ab[], m, n, constraints[], basis[][MAX_COEF], *num_basis;
{
...
/*************
 *
 *    next_combo_a1 -- similar to a, but prevent supersets; not really incremental.
 *
 *************/

int superset_degree(a, b, n)
int a[], b[], n;
...
int next_combo_a1(length,basis,num_basis,constraints,combo,sum,start_flag,combos,np,ss_parm)
int length;
int basis[][MAX_COEF];
int num_basis;
int constraints[];
...
int next_combo_a(length,basis,num_basis,constraints,combo,sum,start_flag)
int length;
int basis[][MAX_COEF];
int num_basis;
int constraints[];
...
int next_combo_b(length, basis, num_basis, constraints, combo, sum, start_flag)
int length;
int basis[][MAX_COEF];
int num_basis;
int constraints[];
...
int next_combo_c(length, basis, n, constraints, combo, sum, start_flag)
int length;
int basis[][MAX_COEF];
int n;
int constraints[];
...
int all_combos(m, n, basis, num_basis, constraints, proc)
int m;
int n;
int basis[][MAX_COEF];
int num_basis;
...
int all_combos_build(m, n, basis, num_basis, constraints, proc)
     int m;
     int n;
     int basis[][MAX_COEF];
     int num_basis;
     ...
     int (*proc)();
{
	...
	for (j = 0; j < m+n; j++) {
		...
		p_term(basis_terms[i][j]->farg->argval);
	    printf("  ");
	    }
	printf("\n");
	}
