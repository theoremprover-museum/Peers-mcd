...
int str_to_sn(char *str, int arity)
{
    ...
    /* Peers_mcd, November 1996: only Peer0 checks arities */
    if (Pid == 0 && 
        save && !save->special_op &&
...
/*************
 *
 *    int is_symbol(sym, str, arity)
 *
 *    Does sym (where sym is t->symbol for some term t)
 *    have name str and arity arity?
 *
 *************/
...
void auto_lex_order(void)
{
    ...
    for (i = 0; i < j; i++) {
        a[i]->lex_val = i + 1 + max;
#if 0
        printf("%d %7s %d %d\n", a[i]->sym_num, a[i]->name, a[i]->arity, a[i]->lex_val);
#endif
        }
 ...
 *
 * imported from old io.c used in previous version of peers (Peers-mcd, October 1996)
 *
 *************/
