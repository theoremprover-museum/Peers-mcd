...
#include "Header.h"	/* Peers-mcd needs it, eqp does not */
#include "Options.h"
...
void init_options(void)
{
    ...
    /* Peers_mcd -- March 1999 */

    f = &(Flags[DIVERSE_SEL]);
    f->name = "diverse_sel";
    f->val = 0;
    
    f = &(Flags[DIVERSE_PICK]);
    f->name = "diverse_pick";
    f->val = 0;

    /* Peers-mcd -- April 2000 */

    f = &(Flags[HEURISTIC_SEARCH]);
    f->name = "heuristic_search";
    f->val = 0;

    /* Peers-mcd -- September 2000 */
    ...
    p = &(Parms[MAX_WEIGHT_SEND]);
    p->name = "max_weight_send";
    p->min = -INT_MAX;
    p->max = INT_MAX;
    p->val = INT_MAX;
   ...
   /* Peers-mcd -- April 2000 */
    ...
    p = &(Parms[HEURISTIC_MEASURE]);
    p->name = "heuristic_measure";
    p->min = 0;
    p->max = 2;
    p->val = -1;
