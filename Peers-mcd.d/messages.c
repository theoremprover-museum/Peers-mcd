 ...
 *
 *    "id weight heuristic_value interpreted_value <length of justification> <justification> <literal> "
 *    where
...
void clause_to_string(Clause_ptr c, char *s, int size)
{
    ...
    sprintf(s, "%d %d %d %d %d ",
	    c->id,
	    c->weight,
	    c->heuristic_value,	                    /* Peers_mcd, April 2000 */
	    c->interpreted_value,
	    i);
...
Clause_ptr string_to_clause(char *s)
{
    ...
    n = next_int(&s);
    c->weight = n;
    n = next_int(&s);	                                                  /* Peers_mcd, April 2000 */
    c->heuristic_value = n;
    n = next_int(&s);
